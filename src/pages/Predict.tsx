...
// Predict the Outbreak - Single-session epidemiological forecasting game
// Player reveals data progressively, predicts earlier for higher multiplier
...
export function Predict() {
  ...
  // Live challenge state — single-session reveal step
  ...
  // Get visible data for live challenge based on reveal step
  const visibleLiveData = getVisibleData(MYSTERY_OUTBREAK_2026, revealStep);
  const currentMultiplier = getMultiplier(revealStep);
  const canRevealMore = revealStep < DATA_REVEAL_STEPS.length - 1;
  ...
  const handleLivePrediction = (prediction: EnhancedPrediction) => {
    ...
    // Calculate score using reveal step multiplier
    const breakdown = calculateEnhancedScore(prediction, MYSTERY_OUTBREAK_2026, revealStep);
  ...
  if (showLiveChallenge) {
    return (
      <GameShell
        ...
        heroSubtitle="Predict the Outbreak"
        showNav={true}
      >
        {/* Confetti celebration */}
        ...
        <div className="predict-workspaceWrap animate-slide-up">
          ...
          {/* Data Reveal Banner - Compact */}
          <LiveChallengeBanner
            revealStep={revealStep}
            weeksAvailable={visibleLiveData.length}
            onRevealMore={canRevealMore ? handleRevealMore : undefined}
            isCompact
          />
              ...
              {liveScore && livePrediction ? (
                <OrnatePanel pad="lg">
                  <div className="text-center mb-6">
                    ...
                    {liveScore.revealBonus.multiplier > 1 && (
                      <div className="mt-2">
                        <span className="pill pill-gold">
                          <Zap size={14} />
                          Includes {liveScore.revealBonus.multiplier}x early prediction bonus!
                        </span>
                      </div>
                    )}
                  </div>
      ...
      <div className="eis-predictWrap animate-slide-up">
        ...
        <OrnatePanel pad="md" className="mt-6">
          ...
          <div className="space-y-2 text-[#3d2b1f] text-sm">
            ...
            <p><strong>Less Data = More Points:</strong> Predict with fewer weeks revealed for a higher score multiplier (up to 3x!)</p>
