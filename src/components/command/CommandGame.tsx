...
export function CommandGame({ scenario, onComplete, onBack }: CommandGameProps) {
  ...
  // Tutorial and toast states — always show on kiosk (walk-up-and-play)
  ...
  // Process event queue — wait for tutorial to finish first
  useEffect(() => {
    if (eventQueue.length > 0 && !currentEvent && !showTutorial) {
      setCurrentEvent(eventQueue[0]);
      setEventQueue(prev => prev.slice(1));
      setGameStatus('paused');
    }
  }, [eventQueue, currentEvent, showTutorial]);

  // Game loop — paused while tutorial is showing
  useEffect(() => {
    if (gameStatus !== 'playing' || showTutorial) return;
  ...
  }, [gameStatus, speed, advanceDay, showTutorial]);
  ...
  // Render briefing screen — tutorial shows here before game starts
  if (gameStatus === 'briefing') {
    return (
      <div className="max-w-2xl mx-auto p-6">
        {/* Tutorial shows during briefing, blocking Start until dismissed */}
        {showTutorial && (
          <TutorialOverlay
